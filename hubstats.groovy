...
preferences {
    ...
    input("fwUpdateCheckPollRate","number", title:"Poll rate (in seconds) for FW Update Check (Default:6000, Disable:0):", defaultValue:6000, submitOnChange:true) 
}
...
def configure() {
    ...
    if(fwUpdateCheckPollRate == null) 
        device.updateSetting("fwUpdateCheckPollRate",[value:6000,type:"number"])
    if(fwUpdateCheckPollRate > 0 ) updateCheck()
}
...
void getJvmHandler(resp, data) {
   // log.debug "in get jvm handler resp = $resp"
    String jvmWork
    List<String> jvmArr = []
    try {
        if(resp.getStatus() == 200 || resp.getStatus() == 207) {
