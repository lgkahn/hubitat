 ...
 *
 ...
 -- lgk feb 2018 add. add offset for humidity as gen 2 sensors dont match levels for gen 1 .
 
 */
metadata {
	definition (name: "Spruce Sensor - lgk", namespace: "plaidsystems", author: "Plaid Systems") {
	...
	tiles {
		...
		main  (["humidity"])
		details(["humidity","maxHum","minHum","temperature","battery","status"])
	}
}
...
private Map getHumidityResult(value) {
    def linkText = getLinkText(device)
    def localvalue = value.toInteger()
 
    def maxHumValue = 0
    def minHumValue = 0 
    
    def localoffset = settings.HumidOffset
    
    if (localoffset == null)
      localoffset = 0
      
    
    log.debug "in get humidresult"
    log.debug "offset = $localoffset"
    log.debug "localvalue = $localvalue"

    // LGk use off but reset if less than 0
     def adjvalueint = localvalue + localoffset
    
     if (adjvalueint < 0)
      adjvalueint = 0
    
    def adjvalue =  adjvalueint.toString()
     
    log.debug "value = $value, offset = $settings.HumidOffset" 
    log.debug "adj humidity value = $adjvalue"
    ...
    def compare = adjvalue.toInteger()
    
    if (compare > maxHumValue) {
        sendEvent(name: 'maxHum', value: adjvalue, unit: '%', descriptionText: "${linkText} soil moisture high is ${vadjalue}%")
        }
    else if (((compare < minHumValue) || (minHumValue <= 2)) && (compare != 0)) {
        sendEvent(name: 'minHum', value: adjvalue, unit: '%', descriptionText: "${linkText} soil moisture low is ${adjvalue}%")
        }    
    ...
    return [
    	...
    	value: adjvalue,
    	unit: '%',
        descriptionText: "${linkText} soil moisture is ${adjvalue}%"
    ]
}
