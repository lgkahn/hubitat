 ...
 *
 * lgk kahn@lgk.com Initial Version
 *
 * 
 */

metadata {
	definition (name: "TeslaVehicle", namespace: "lgkahn", author: "Larry Kahn") {
		capability "Actuator"
		capability "Battery"
// not supported	capability "Geolocation"
     ...
     //   attribute "front_drivers_door", "string"
      ...
      //  command "transitionAccessToken"
        ...
        command "steeringWheelHeaterOn"
        command "steeringWheelHeaterOff"
	...
	if(data) {
        ...
        if (data.vehicleState) {
           if (debugLevel == "Full") log.debug "vehicle state = $data.vehicleState"
            ...
            // no door or frunk etc status in tesla fi
            /*
...
def heat() {
	if (debugLevel != "None") log.warn "Not Supported"
	// Not supported
}
...
def setThermostatMode(mode) {
	if (debugLevel != "None") log.warn "Executing 'setThermostatMode'"
	switch (mode) {
    	case "auto":
        	auto()
            break
        case "off":
            ...
            break
        default:
        	log.warn "setThermostatMode: Only thermostat modes Auto and Off are supported"
    }
}
