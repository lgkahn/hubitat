 ...
 *
 *     lgk: v1.4.3   add retries after unauthorized and read failures
 *     lgk: v1.4.1   add html query and parsing to get whole house/steam humidifier info and put in attributes
 *
 *     lgk: v1.4.0   changed tcc line
 *  lgkahn: v1.3.15  modified again by lgkahn 11/20 first the heating cooling was not working right.. 
 *                   changed back to using equipment status.. just becuase the fan is on does not mean it is heating.. 
...
def getStatusHandler(resp, data) {
	if(resp.getStatus() == 200 || resp.getStatus() == 207) {
       ...
       lgk end old code  */
        ...
        /* lgk change here new code

           if you have if else if here as specified it will never go into the else as the fan is always running when you are humidifying//
           operating state of fan only is incorrect operating state reads 0 when the fan is off or on, at least in my 2 thermostats, so fan only is a misnomer.
           if you really want to set fan only you need to
           1. check that operating state = 0 and 2 that fanisrunning = true and 3 that you dont have humidifier enabled
         */
     ...
     httpGet(params) { response ->
        ...
        data.split("\n").each {
        	...
        	if (it.contains("CancelMin")) {
            	...
            	log.debug "Got Current humidifier new = $HumMax"
        	}
        }
