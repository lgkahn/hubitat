 ...
 *
 * modified by lgkahn to add switch so alexa can use and polling if needed 2014
 */
metadata {
	definition (name: "Z-Wave Garage Door Opener - LGK", namespace: "lgkapps", author: "SmartThings") {
		capability "Actuator"
...
def zwaveEvent(BarrierOperatorReport cmd) {
	def result = []
	def map = [ name: "door" ]
    log.debug "in zwave event barrierstate = $cmd.barrierState"
