...
private void sensorMapping(Map data) {
  ...
  String[] sensorMap =  ["WH69", "WH25", "WH26", "WH31", "WH40", "WH41", "WH51", "WH55", "WH57", "WH80", "WH34","WFST", "WS90", "WH54"];
...
private Boolean attributeUpdate(Map data, Closure sensor) {
  ...
  data.each {
    ...
    case "humidity":
     // lgk 5/25 new vpd 
    case "vpd":    updated = sensor(it.key, it.value, 2);
      break;
    ...
    case ~/pm25_avg_24h_ch([1-4])/:
     ...
     case ~/ldsbatt([1-4])/:
     case ~/ldsheat_ch([1-4])/:
      updated = sensor(it.key, it.value, 13,  java.util.regex.Matcher.lastMatcher.group(1).toInteger());
      break;
...
{  
   ...
   logDebug("in nscallback data = ${resp.data}")
     ...
     if ((ddnsname != null) && (ddnsname != ""))
       {
        ...
        Map params = [
        uri: "https://8.8.8.8/resolve?name=$ddnsname",
